...
ipcRenderer.on("delete-drugs", function (e, id) {
  ...
  let drugID = tr.getAttribute("analyseID"); //hada hwa id nta3 dwa li baghi t7adfo
...
ipcRenderer.on("data-analyseEdit", function (e, dataAnqlyseEdit) {
  ...
  db.run(
    sql,
    [dataAnqlyseEdit[0], dataAnqlyseEdit[1], dataAnqlyseEdit[2], id],
    (err) => {
      if (err) return console.log(err);
    }
  );
...
ipcRenderer.on("data-analse", function (e, dataAnalyse) {
  ...
  db.run(
    `INSERT INTO analyses (analyse, name, fullName, description) values(?,?,?,?)`,
